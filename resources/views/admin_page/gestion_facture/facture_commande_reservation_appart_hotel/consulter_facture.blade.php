...
<div id="page-content">
    ...
    <div class="content-header">
        <div class="header-section">
            <h1>
                <i class="gi gi-table"></i>Gestion des factures de commande de réservation d'appartement / Hotel<br><small>Consulter les factures ici !</small>
            </h1>
        </div>
    </div>
    ...
    <div class="block full">
        ...
        <div class="table-responsive">
            ...
            <table id="example-datatable" class="table table-striped table-vcenter table-condensed table-bordered">
                ...
                <tbody>
                    @foreach($facture_commande_reservation_appart_hotel as $item)
                    <tr>
                        <td>#CRAH{{$item->id}}</td>
                        <td>{{$item->reservations_appart_hotel->type_reservation}}</td>
                        <td>{{$item->reservations_appart_hotel->users->name}}</td>
                        <td><a href="javascript:void(0)" class="label label-primary">{{$item->reservations_appart_hotel->created_at->format('d/m/Y')}}</a></td>
                    ...
                    <div class="modal fade" id="pages_edit_{{$item->id}}" role="dialog">
                        <div class="modal-dialog modal-dialog-centered modal-md">
                            <div class="modal-content">
                                ...
                                <form action="{{route('facture.commande_reservation_appart_hotel.generer')}}" method="get">
                                    @csrf
